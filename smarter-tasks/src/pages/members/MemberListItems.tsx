      ...
      {members.map((member: any) => (
          <div key={member.id} className=" block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 member">
                    <div className=" member flex justify-between items-center">
                        <h5 className=" member mb-1 text-xl font-medium tracking-tight text-gray-900 dark:text-white">{member.name}</h5>
                        <button className="member" onClick={() => deleteMember(dispatchMembers, member.id)}>
